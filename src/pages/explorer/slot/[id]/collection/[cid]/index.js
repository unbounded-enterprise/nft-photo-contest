...
const ExploreCollectionPage = () => {
  ...
  useEffect(() => {
    if (collectionId) {
      getNFTs({
        collectionId: chosenCollection.collectionId,
        serials: nftSearch,
      })
        .then((nfts) => {
          setNFTs(nfts);
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:image"
          content={chosenCollection.collectionImage}
          key="ogimage"
        />
        <meta
          property="og:site_name"
          content={chosenCollection.collectionName}
          key="ogsitename"
        />
        <meta
          property="og:title"
        ...
        <meta
          ...
          content={
            "Vote for " +
            chosenCollection.tags[0] +
            "'s entry into the NFT Photo Contest!"
          }
          key="ogdesc"
        />
      </Head>
      <Box sx={{ backgroundColor: "none", py: 5 }}>
        ...
        >
          <Grid container spacing={2}>
            ...
            {user && voted ? (
              <Grid item xs={12} sx={{ backgroundColor: "none" }}>
                <Typography>You&apos;ve voted for this collection!</Typography>
              </Grid>
            ) : (
              <></>
            )}
...
ExploreCollectionPage.getLayout = (page) => <MainLayout>{page}</MainLayout>;
...
const getNFTs = async ({ collectionId, serials, from, to }) => {
  ...
  if (collectionId) {
    if (serials) {
      nftsObject = await axios.post("/api/collection/nfts", {
        collectionId,
        idOnly: false,
        serials,
      });
