...
export function getExpressionValue(
  ...
  const expressionValue = expressionValues?.find(
    (expressionVal) =>
      expressionVal?.expression?.expressionName === expressionName &&
      expressionVal?.expressionAttribute?.expressionAttributeName ===
        expressionAttributeName
  )?.value;
...
export function parseNFT(nft, expression) {
  if (!nft) {
    return {
      ...
      parsedAtlas: null,
      parsedPng: null,
      parsedImageExpression: null,
      parsedAudioExpression: null,
    };
  }
  ...
  const parsedImageExpression = getExpressionValue(
    expressionValues,
    expression,
    "Image"
  );
  const parsedAudioExpression = getExpressionValue(
    expressionValues,
    expression,
    "Audio"
  );
...
export default function DisplayNFT({
  ...
  }, // (spine)=>void will be called once the spine is loaded and have the spine as parameter, use the spine i.e. to call animations manually
  onAudioLoaded = (audioFile) => {
    console.log("audio file: ", audioFile);
  }, // will be called if the expression chosen is containing an audioFile
  ...
  useEffect(() => {
    const {
      ...
      parsedAudioExpression,
    } = parseNFT(assetlayerNFT, expression);
  ...
  function resourcesLoaded(resources) {
    ...
    const nftSpine = new PIXISPINE.Spine(
      resources[assetlayerNFT.nftId + expression].spineData
    );
    if (nftSpine) {
      setSpine(nftSpine);
    }
  ...
  async function loadPixiNFT() {
    ...
    if (!resources || !resources[assetlayerNFT.nftId + expression]) {
      ...
      resources[assetlayerNFT.nftId + expression] = await PIXI.Assets.load({
        src: spineJson,
        data: { spineAtlas: atlas },
      });
          ...
          <>
            {spine && (
              <>
                <SpineDisplay
                  spines={[spine]}
                  nftSizePercentage={nftSizePercentage}
                />
              </>
            )}
          </>
          ...
          <AudioDisplay
            ...
            backgroundImage={
              soundBackground === "Menu View"
                ? getExpressionValue(
                    assetlayerNFT.expressionValues,
                    "Menu View",
                    "Image"
                  )
                : soundBackground
